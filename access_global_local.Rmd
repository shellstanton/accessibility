...
bbymin <- -11.29
bbymax <- -10.73
## 'EarthEngine Object: Geometry' created within markdown
...
library(readxl)
mwi_mfl <- read_excel("./malawi_health_facility_data/malawi.xlsx")
# Check
# View(mwi_mfl)
...
mwi_mfl_spdf <- SpatialPointsDataFrame(coords = mwi_mfl[c("LATITUDE", "LONGITUDE")],
                                       proj4string = crs(friction_surface_motor),
                                       data = mwi_mfl)
...
### Create SpatialPoints object (with CRS of 'friction_surface_motor')
mwi_mfl_sp <- SpatialPoints(coords = mwi_mfl[c("LATITUDE", "LONGITUDE")],
                            proj4string = crs(friction_surface_motor))
...
# Convert mwi_mfl_spdf_sf to sf object (geometry type: POINT geometry)
mwi_mfl_spdf_sf <- st_as_sf(mwi_mfl_spdf)
mwi_mfl_spdf_sf
# Simple feature collection with 1028 features and 11 fields
# geometry type:  POINT
# dimension:      XY
# bbox:           xmin: -542183 ymin: 1 xmax: -1 ymax: 70989710
# CRS:            +proj=longlat +datum=WGS84 +no_defs


## bbox needs to be cropped to aoi


# Crop 
mwi_mfl_spdf_sf_crop <- st_crop(mwi_mfl_spdf_sf, 
                                c(xmin = bbxmin, 
                                  xmax = bbxmax, 
                                  ymin = bbymin, 
                                  ymax = bbymax))
## Warnings:
# bounding box has potentially an invalid value range for longlat data although coordinates are longitude/latitude,
# st_intersection assumes that they are planar attribute variables are assumed to be spatially constant throughout all geometries

# Tried with st_bbox(healthfac): Same errors
# Tried with aoi_polygon: Same errors:
                           
mwi_mfl_spdf_sf_crop
