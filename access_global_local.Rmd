...
# Subset by < 30 mins 
zero_thirty <- subset(lcm_df, mins < 30)

# Check
View(zero_thirty)

# Remove 'layer' collumn to give just three rows (needed for 'rasterfromXYZ' function)
zero_thirty$layer <- NULL

# Check with plot
ggplot()+geom_raster(data=zero_thirty, aes(x = x, y = y, fill = cut(mins, c(0,30,max(mins)))))+
    scale_fill_brewer(palette = "YlGnBu")+
    geom_sf(data=q$osm_lines, colour="darkgrey", alpha=0.3)+
    geom_sf(data=healthfac, size=2, colour="red")+
    guides(fill=guide_legend(title="Time (mins)"))
...
# Create raster
zero_thirty_raster <- rasterFromXYZ(zero_thirty)

# Check
View(zero_thirty_raster)
zero_thirty_raster
plot(zero_thirty_raster)


test <- rasterToPolygons(zero_thirty_raster, dissolve = TRUE)
...
# Save as shapefile
writeOGR(test, dsn = "test", layer = "test", driver = "ESRI Shapefile")
...
# Check CRS
st_crs(zero_thirty_raster)
# CRS = NA

# Set CRS to WGS84
zero_thirty_raster <- crs("+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0")
...
# Currently 'rasterbrick'. Create matrix to make 'rasterlayer'
zero_thirty_matrix <- as(zero_thirty, "matrix")

# Check
View(zero_thirty_matrix)
zero_thirty_matrix

# Check CRS
st_crs(zero_thirty_matrix)
# CRS = NA

# Set CRS to WGS84
zero_thirty_matrix <- crs("+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0")
...
zero_thirty_polygon <- rasterToPolygons(zero_thirty_raster)
...
### Subset data.frame by time -> create .csv -> convert .csv to polygon ?
...
# Check with plot
ggplot()+geom_raster(data=zero_thirty, aes(x = x, y = y, fill = cut(mins, c(0,30,60,120,180,240,300,max(mins)))))+
    scale_fill_brewer(palette = "YlGnBu")+
    geom_sf(data=q$osm_lines, colour="darkgrey", alpha=0.3)+
    geom_sf(data=healthfac, size=2, colour="red")+
    guides(fill=guide_legend(title="Time (mins)"))
...
# Create .csv
write.csv(zero_thirty, "./outputs/zero_thirty.csv")

# Read in .csv
zero_thirty_csv <- read.csv("./outputs/zero_thirty.csv")

# Check CRS
st_crs(zero_thirty_csv)
# CRS = NA.

# Convert data.frame to spdf
zero_thirty_spdf <-SpatialPointsDataFrame(zero_thirty_csv, zero_thirty_csv)

# Assign CRS (WGS84)
zero_thirty_spdf <- crs("+proj=utm +zone=18 +datum=WGS84 +units=m +no_defs +ellps=WGS84 +towgs84=0,0,0" )

# Check CRS
st_crs(zero_thirty_spdf)
# CRS = NA.
