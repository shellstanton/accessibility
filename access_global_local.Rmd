...
### Option_3a: Download population rasters from WorldPop, subset time-boundary data, create polygon(s) and extract from population rasters using polygon(s) (Working). 
...
### Option_3b: Extract from population rasters using polygon(s) (30 min) (Working)
```{r population_30_min, include=FALSE}


## Compute a 30 minute buffer zone around the health facilities and intersect that with population density ???
...
### Option_3b: Extract from population rasters using polygon(s) (60 min) (Working)
```{r population_60_min, include=FALSE}
...
### Option_3c: Extract from population rasters using reclasified rasters (rather than polygons) ???
```{r population_60_min, include=FALSE}
...
##########
rcl_matrix <- data.frame(lower_limit = c(0, 30, 60, 90, 120, 150, 180, 240),
                         upper_limit = c(30, 60, 90, 120, 150, 180, 240, 300),
                         reclassification = c(1, 2, 3, 4, 5, 6, 7, 8)
                         )
View(rcl_matrix)

##########
rcl_matrix_2 <- data.frame(time_boundary = c (0, 30, 60, 90, 120, 150, 180, 240, 300),
                           reclassification = c(1, 2, 3, 4, 5, 6, 7, 8, 9)
                           )

View(rcl_matrix_2)

##########
rcl_matrix_3 <- c(0, 30, 1,
                  30, 60, 2,
                  60, 90, 3,
                  90, 120, 4,
                  120, 150, 5,
...
rcl_matrix_3 <- matrix(rcl_matrix_3, ncol=3, byrow=TRUE)
View(rcl_matrix_3)

# Create new raster of leastcost_motor time boundaries 

# Reclassify 'XXX' with 'rcl_matrix' 
lcm_pop_data_rcl <- reclassify(lcm_pop_data, rcl_matrix_3, include.lowest=TRUE)
...
lcm_rcl_zone <- zonal(pop_data, lcm_rcl, fun = sum)
...
### Test mask, ignore.
```{r test_mask, include=FALSE}

# Test mask?
test_mask <- mask(pop_data, thirty_min_polygoy_agg)
plot(test_mask)
