...
# * 'devtools', needed to download some packages from GitHub
# * 'wpgpDownloadR' and 'wpgpCovariates', needed for downloading raster datasets from the WorldPop FTP
# * 'afrimapr/afrihealthsites' package, needed for downloading healthcare facility data from AfriMapR/WHO
...
2. Setup: Country of interest (coi), used to obtain:
* WHO health facility data, downloaded via AfriMapR within script
* Population data, downlaoded via WorldPop within script
...
3. Setup: Area of interest (aoi)
* Detail limits for a bounding box (lat/long maximum and minimum values, using WGS84 projection) to be used as area of interest (aoi)
OR
...
4. Setup: Landsat 8 parameters
* Read in Landsat 8 data, obtained using Google Earth Engine (GEE) *download from within R?*
* Define start and end dates to filter Landsat 8 data by
This data will be used to calculate the median NDVI per pixel. NDVI values will then be used to generate friction surface and calculate travel times
...
*OSM national speed limit data needed*
```{r setup_walking_travel_speeds}
...
# Run 'population_data' function; constructed to return dataframe of available covariates downloaded from WorldPop FTP (for country of interest) based on 'coi', defined in setup stage 5: Health facility data
population_data <- 
...
*Needed in setup? - To setup GoogleDrive account?*
```{r, eval=F}
...
Downloads data and locations of documented health facilities within country of interest and crops to area of interest
```{r}
...
HF_list_WHO <- as.data.frame(mwi_healthfac_who_spdf_cropped)
View(HF_list_WHO)
kable(HF_list_WHO)
...
# mwi_mfl_cropped
# SpatialPointsDataFrame
HF_list_MFL <- as.data.frame(mwi_mfl_cropped)
kable(HF_list_MFL)
...
# Check
FS_output <- kable(lcm_rcl_zone_df,
                   caption = "Percent (%) XYZ...")
...
bar_plot_df <- data.frame(c("< 30 minutes", "< 1 hour", "< 3 hours", "< 6 hours", "< 12 hours", "< 24 hours"),
                          lcm_rcl_zone_df$"% Population")
...
FS_barplot <- 
  ...
  ggplot(data = bar_plot_df, aes(x = bar_plot_df$Zone, y = bar_plot_df$`% Population`)) +
  geom_bar(aes(fill = "% Population"), width = 0.4, position = position_dodge(width=0.5), stat="identity") +
  scale_x_discrete(limits = bar_plot_df$Zone) +
  theme_light() +
...
colnames(bar_plot_df)
names(bar_plot_df)[1] <- "Zone"
names(bar_plot_df)[2] <- "% Population: WHO database"
names(bar_plot_df)[3] <- "% Population: MFL database"
colnames(bar_plot_df)

# Check  
bar_plot_df  

# Melt using `melt()`function from `reshape2` package
# library(reshape2)
bar_plot_df <- melt(bar_plot_df)
# Check
View(bar_plot_df)
...
FS_barplot <- 
  ...
  ggplot(data = bar_plot_df, aes(Zone, value)) +
  geom_bar(aes(fill = variable), width = 0.4, position = position_dodge(width=0.5), stat="identity") +
  scale_x_discrete(limits = bar_plot_df$Zone[1:6]) +
  theme_light() +
  ylab("Percent (%) of population") +
  xlab("Time-boundary category (Travel time from closest-proximity health facility)") +
  ggtitle("Percent (%) of population within area of interest residing within pre-defined time-boundaries of 
the closest-proximity health facility according to health facility data obtained from World Health 
Organisation (WHO) and Master Health Facility (MFL) Registry of Malawi databases") +
  theme(plot.title = element_text(size = 10, hjust = 0.5),
        legend.title = element_blank(),
        axis.title=element_text(size = 10),
        axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  scale_fill_discrete(labels = c("WHO database", "MFL database"))
...
MAP comparison Bar Plot (*Delete*)
```{r}


# Create  dataframe
bar_plot_MAP_df <- data.frame(c("< 30 minutes", "< 1 hour", "< 3 hours", "< 6 hours", "< 12 hours", "< 24 hours"),
                          c(37, 65.4, 90.3, 97.3, 99.9, 100),
                          c(96.3, 99.2, 100, 100, 100, 100))

# Check  
bar_plot_MAP_df  

# Rename columns 
colnames(bar_plot_MAP_df)
names(bar_plot_MAP_df)[1] <- "Zone"
names(bar_plot_MAP_df)[2] <- "% Population: Our friction surface"
names(bar_plot_MAP_df)[3] <- "% Population: Malaria Atlas Project friction surface"
colnames(bar_plot_MAP_df)
...
# Melt using `melt()`function from `reshape2` package
# library(reshape2)
bar_plot_MAP_df <- melt(bar_plot_MAP_df)
# Check
View(bar_plot_MAP_df)


# Create bar plot
FS_MAP_barplot <- 
  
  ggplot(data = bar_plot_MAP_df, aes(Zone, value)) +
  geom_bar(aes(fill = variable), width = 0.4, position = position_dodge(width=0.5), stat="identity") +
  scale_x_discrete(limits = bar_plot_df$Zone[1:6]) +
  theme_light() +
  ylab("Percent (%) of population") +
  xlab("Time-boundary category (Travel time from closest-proximity health facility)") +
  ggtitle("Percent (%) of population within area of interest residing within pre-defined time-boundaries 
  of the closest-proximity health facility according to our friction surface and the Malaria Atlas 
  Project friction surface when using health facility data obtained from the Master Health Facility 
  (MFL) Registry of Malawi") +
  theme(plot.title = element_text(size = 10, hjust = 0.5),
        legend.title = element_blank(),
        axis.title=element_text(size = 9),
        axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  scale_fill_discrete(labels = c("Our friction surface", "Malaria Atlas Project 
friction surface"))
...
bound_districts <- 
  st_read("Bounds/mwi_1.gpkg", quiet = TRUE)

plot(st_geometry(bound_districts))
