...
# Return list of all packages being used:
...
Option_4: Download population raster from the WorldPop FTP, determine population within time-boundary zones from population rasters using reclasified time-boundary data (rather than polygons).
```{r WorldPop_packages}

## Download, install and load wpgpDownloadR: an R Package interface for downloading raster datasets from WorldPop FTP.

# Requires installation of "devtools" package
...
Obtain ISO3 (country code) for country of interest (Malawi).
```{r ISO3_country_code}

## Download ISO3 (country code) dataframe to obtain ISO3 info

# The 'wpgpListCountries' function will return a dataframe with all ISO3 available on WorldPop ftp server.
...
## The 'wpgpListCountryDatasets' function will return a dataframe of available covariates from WorldPop FTP for a country of interest

# Download dataset of avaliable covariates for country of interest
...
# Download  dataset (.tif) for country and covariate of interest (based on ISO3 and covariate name)
wpgpGetCountryDataset(ISO3 = ISO3,
                      covariate = covariate, # This needs to be chosen?
                      destDir = "./WorldPop")

# Note: Downloaded .tif is ~50 MB
...
Reclassify 'leastcost_motor' to chosen time-boundary categorical zones.
```{r reclassify}

## Create rasterlayer from 'lcm_df' dataframe
# 'rasterfromXYZ' function works only with three columns, so remove 'layer' column from 'lcm_df'
lcm_df$layer = NULL
# Check
# View(lcm_df)

# Create reaster from 'lcm_df' dataframe
lcm_raster <- rasterFromXYZ(lcm_df)
# Check
# plot(lcm_raster)

## Create matrix of time-boundary categories (to resample 'lcm_raster')
## Categories: 
# * 0 - 30 minutes
# * 30 minutes - 1 hour
# * 1 hour - 3 hours
...
Determine population within each time-boundary category. Summate the data to give total number of individuals within chosen time-boundary categorical zones and calculate % of total population within chosen time-boundary categorical zones.
```{r determine_population_within_time-boundary_zones}

# Determine population totals (within 'pop_data' raster), within time_boundary categorical zones (within 'lcm_pop_data_rcl') using 'zonal' function 
