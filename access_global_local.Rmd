...
## Download, install and load wpgpDownloadR: an R Package interface for downloading raster datasets from the WorldPop FTP.
...
OR
* Specify district name to define area of interest
...
bbymin <- -11.29
bbymax <- -10.73
## Polygon created within markdown
...
```
*Need to look into specifying district name*
...
# These can be changed according to expected changes in walking speeds, i.e., during the wet season, etc.
...
# These can be changed according to expected changes in travel speeds, i.e., during the wet season, etc.

```


5. Setup: Population parameters
...
* Select and set covariate of interest by viewing dataframe of available covariates downloaded from WorldPop FTP (for country of interest), which is used to download population data from the WorldPop FTP
*Can this be done as function?*
```{r setup_population_parameters}

# * Set country of interest (coi) using British English spelling
coi <- "Malawi"

# * Obtain ISO3 country code for country of interest (coi)
# The 'wpgpListCountries' function will return a dataframe with all ISO3 country codes available on WorldPop FTP server
...
# View data and check for/select covariate of interest
View(covariates_df)
...
*Changed to installation via CRAN within setup; remove this chunk?*
```{r}
...
# Download  dataset (.tif) for country and covariate of interest (based on ISO3 and covariate name set during setup stages)
